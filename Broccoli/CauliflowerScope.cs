    ...
    public class CauliflowerScope : Scope {
        ...
        public override IScalar this[ScalarVar s] {
            get => Scalars.ContainsKey(s.Value)
                ? Scalars[s.Value]
                : Parent?[s] ??
                  (Lists.ContainsKey(s.Value)
                      ? Lists[s.Value].ScalarContext()
                      : Parent?[new ListVar(s)]?.ScalarContext() ??
                        (Dictionaries.ContainsKey(s.Value)
        ...
        public override IList this[ListVar l] {
            get => Lists.ContainsKey(l.Value)
                ? Lists[l.Value]
                : Parent?[l] ??
                  (Scalars.ContainsKey(l.Value)
                      ? Scalars[l.Value].ListContext()
                      : Parent?[new ScalarVar(l)]?.ListContext() ??
                        (Dictionaries.ContainsKey(l.Value) ? Dictionaries[l.Value].ListContext() : Parent?[new DictVar(l)]?.ListContext()
                        )
                  );
        ...
        public override IDictionary this[DictVar d] {
            get => Dictionaries.ContainsKey(d.Value)
                ...
                : Parent?[d] ??
                  (Scalars.ContainsKey(d.Value)
                      ? Scalars[d.Value].DictionaryContext()
                      : Parent?[new ScalarVar(d)]?.DictionaryContext() ??
                        (Lists.ContainsKey(d.Value) ? Lists[d.Value].DictionaryContext() : Parent?[new ListVar(d)]?.DictionaryContext()
                        )
                  );
