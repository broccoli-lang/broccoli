    ...
    public class SExpression : ISExpressible {
        ...
        public SExpression(List<Token> tokens) {
            if (tokens.First().Type != TokenType.LeftParen)
                throw new Exception("First token in sexp must be left paren!");
            if (tokens.Last().Type != TokenType.RightParen)
                throw new Exception("Last token in sexp must be right paren!");
        ...
        public static IEnumerable<(int, int)> GetSexpRangesList(List<Token> tokens) {
            ...
            while (true) {
                parenIndex = tokens.Select(t => t.Type).ToList()
                    .IndexOf(TokenType.LeftParen, parenIndex);
                if (parenIndex == -1) return sexpRanges;

                int rightParenIndex =
                    MatchingCloseParenIndex(tokens.Skip(parenIndex + 1).ToList()) + parenIndex + 1;
